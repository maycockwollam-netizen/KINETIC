...
from tasks.models import Plan, PlanStep, StepStatus, Task, TaskFailure
from tasks.observer import Observation, Observer
...
from tasks.policies import FailureClass, RetryLimits, VerificationOutcome
from tasks.recovery import RecoveryCoordinator, failure_context_prompt
from tasks.states import REPLAN_SOURCE_STATES, TaskState
from tasks.verifier import VerificationResult, Verifier

if TYPE_CHECKING:
    from intelligence.diff import ChangeAnalyzer
    from intelligence.models import RepairOutcome
    from intelligence.repair import RepairCoordinator
    from intelligence.review import FinalReviewer, ReviewResult
...
class StepRunner(Protocol):
    ...
    Implementations wrap :class:`~agent.session.AgentSession` (live
...
class ExecutionController:
    ...
    async def _run_step(self, task: Task, plan: Plan, step: PlanStep) -> list[Observation]:
        ...
        while True:
            ...
            from observability.metrics import METRIC_STEPS_EXECUTED
    ...
    async def _verify_task(self, task: Task, plan: Plan, observations: list[Observation]) -> VerificationResult:
        ...
        from observability.metrics import METRIC_VERIFICATION_ATTEMPTS
    ...
    async def _attempt_repair(
        ...
        if self.repair_coordinator is None:
            return None
        from observability.metrics import METRIC_REPAIR_ATTEMPTS
