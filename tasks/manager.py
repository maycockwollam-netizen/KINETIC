...
State transitions are validated against :mod:`tasks.states`; invalid
transitions raise :class:`~errors.TaskStateError`. Cancellation is
...
from tasks.models import Task, TaskFailure
from tasks.states import TaskState, is_terminal, require_transition, transition_allowed
...
class TaskManager:
    ...
    def create(
        ...
        *,
        ...
        if self._metrics is not None:
            from observability.metrics import METRIC_TASKS_STARTED
    ...
    def mark_completed(self, task_id: str) -> Task:
        ...
        if self._metrics is not None:
            from observability.metrics import METRIC_TASK_DURATION, METRIC_TASKS_COMPLETED
    ...
    def mark_failed(self, task_id: str, *, failure: TaskFailure) -> Task:
        ...
        if self._metrics is not None:
            from observability.metrics import METRIC_TASKS_FAILED
    ...
    def _audit_cancel(self, task: Task, reason: str) -> None:
        ...
        if self._metrics is not None:
            from observability.metrics import METRIC_TASKS_CANCELLED
