...
When verification fails, the :class:`RepairCoordinator`:
  ...
  2. analyzes it (via :class:`~kinetic.intelligence.analyzer.FailureAnalyzer`),
  ...
  5. re-runs verification (through the existing :class:`~kinetic.tasks.verifier.Verifier`
...
class RepairRunner(Protocol):
    ...
    Implementations wrap :class:`~kinetic.agent.session.AgentSession.query`
