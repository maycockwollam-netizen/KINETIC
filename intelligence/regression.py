...
The checker reuses the existing :class:`~tasks.verifier.Verifier`
(which routes through ``Environment.exec`` and the permission boundary), and
the Phase 2 :func:`~tasks.verifier.command_for_manifest` to resolve the
...
from intelligence.models import RegressionResult
