...
A runtime maps the abstract :class:`~environment.config.EnvironmentConfig`
onto its native primitives:

    create  -> provision an isolated execution context
    start   -> make it runnable (no-op for some runtimes)
    exec    -> run one :class:`~environment.process.ProcessSpec` inside it
    stop    -> stop the context (processes terminated)
    destroy -> release all resources
    inspect -> report runtime-specific status

Each method may raise :class:`~errors.SandboxError` /
:class:`~errors.RuntimeUnavailableError`. Critically, a runtime must
...
from environment.process import ProcessResult, ProcessSpec
